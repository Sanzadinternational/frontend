...
export default function DemoPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/V1/admin/AllAdminRecords", {
        method: "GET",
        headers: {
  ...
  const handleDelete = async (email: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/V1/admin/DestroyAdmin/${email}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },

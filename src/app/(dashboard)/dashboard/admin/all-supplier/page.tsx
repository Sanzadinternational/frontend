...
async function getData(): Promise<User[]> {
  const response = await fetch(
    "http://localhost:8000/api/V1/admin/AllGetSuppliers",
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
async function getUserDetails(email: string) {
  const response = await fetch(
    `http://localhost:8000/api/V1/admin/SupplierSingleView/${email}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
export default function DemoPage() {
  ...
  const handleAction = async (email: string, status: number) => {
    try {
      const response = await fetch(
        `http://localhost:8000/api/V1/admin/ChangeSupplierApprovalStatus/${email}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",

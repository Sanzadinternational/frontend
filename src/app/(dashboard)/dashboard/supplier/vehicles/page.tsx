...
export default function VehiclePage() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const data = await fetchWithAuth(`${API_BASE_URL}/supplier/dashboard`);
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      ...
      try {
        setLoading(true);
        const response = await fetch(`${API_BASE_URL}/supplier/getCarDetails/${user.userId}`);
        if (!response.ok) throw new Error("Failed to fetch vehicles");
        const data = await response.json();
                ...
                cell: ({ row }) => (
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuLabel>Actions</DropdownMenuLabel>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem onClick={() => router.push(`/dashboard/supplier/AddVehicle?id=${row.original.uniqueId}`)}>
                        Edit

...
//                       {userData?.Company_name?.slice(0, 2)?.toUpperCase() ||
//                             "NA"}
//                       </AvatarFallback>
//                     </Avatar>
//                     <div className="grid flex-1 text-left text-sm leading-tight">
...
export default function AppSidebar({
  ...
  if (rolename === 'admin') {
    roleData = roleData.map(roleItem => ({
      ...
      roleItems: roleItem.roleItems.map(item => {
        ...
        // Remove entire sections if no permissions exist for any items
  ...
  //     ...roleItem,
  //     roleItems: roleItem.roleItems.map(item => {
  //       // For items with sub-items, filter those based on permissions
  //       if (item.items && item.items.length > 0) {
  //         return {
  //           ...item,
  //           items: item.items.filter(subItem => {
  //             // Check if the subItem has a 'show' property and if the user has that permission
  //             return !subItem.show || userData[subItem.show];
  //           })
  //         };
  //       }
  //       return item;
  //     }).filter(item => {
  //       // Filter out top-level items if they have no visible sub-items
  //       if (item.items && item.items.length === 0) return false;
  //       return true;
  //     })
  //   }));
